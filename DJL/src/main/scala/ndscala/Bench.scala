...
var w0:DJLNDArray[Float, (TT, TD, TENKXTENK)] = - moreOnes
  //((origW0:DJLNDArray[Float, (TT, TD, TENKXTENK)]) - (moreOnes:DJLNDArray[Float, (TT, TD, TENKXTENK)])) / (more10ks:DJLNDArray[Float, (TT, TD, TENKXTENK)])
...
def train = {
//     val future = async {
      val l1:DJLNDArray[Float, (TT, TD, TENKXTENK)] =  (x.matmul(w0)).sigmoid() // one / ((-(x dot w0)).exp() + one)
      val l2:DJLNDArray[Float, (TT, TD, TENKXONE)] = (l1.matmul(w1)).sigmoid() // one / ((-(l1 dot w1)).exp() + one)

      val error = (y) - l2
//      println("error: " + error.data.sum)
      val l2Delta = (error) * (l2 * ((ones) - (l2)))
      val l1Delta =  ((l2Delta).matmul(w1.transpose:DJLNDArray[Float, (TT, TD, ONEXTENK)])) * ((l1:DJLNDArray[Float, (TT, TD, TENKXTENK)]) * ((moreOnes - (l1:DJLNDArray[Float, (TT, TD, TENKXTENK)]))))

      
      //Simulate in-place += op here 
      
      w0 = w0 + ((wrap(x.transpose).matmul(l1Delta))) //*moreLrs)
      w1 = w1 + ((wrap(l1.transpose).matmul(l2Delta))) //*lrs)
//   
      println(w0)
      println(w1)
}
